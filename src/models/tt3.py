...
class ClassTTUniformClassifier(nn.Module):
    """
    Class-specific TT classifier with uniform positive initialization.

    This is the same model structure as ClassTTClassifier in tt2.py, but the
    TT cores are initialized uniformly between 0.9 and 1.1 instead of near zero.
    The goal is to test whether TT training improves when the initial matrix
    products do not immediately shrink toward zero.
    """
    ...
    def __init__(
        ...
        if not feature_dims:
            raise ValueError("ClassTTUniformClassifier needs at least one feature")
        if rank < 1:
    ...
    def reset_parameters(self) -> None:
        for core in self.feature_cores:
            nn.init.uniform_(core, a=0.9, b=1.1)
